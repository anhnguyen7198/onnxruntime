    ...
    )
{
    ...
    for (; Channels > 0; Channels--) {
        ...
        *Output++ = std::max(std::min(std::numeric_limits<T8Bits>::max(), v),
                             std::numeric_limits<T8Bits>::lowest());
    }
}
    ...
    )
{
    ...
    for (; Batch > 0; Batch--) {
        ...
        for (size_t c = 0; c < Channels; ++c) {
            ...
            int32_t v =
                static_cast<int>(std::nearbyintf(AccumulateBuffer[c] * scale)) + ZeroPointOutput;
            *batch_output++ = std::max(std::min(std::numeric_limits<T8Bits>::max(), v),
                                       std::numeric_limits<T8Bits>::lowest());
        }
    }
}
